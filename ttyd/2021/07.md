...
使用上还是太熟悉windows系统了，所有软件也配置的差不多了！

2012数据中心版不知道为什么不能安装显卡的驱动，主显卡是英特尔 HD Graphics 530 ( 戴尔 )，现在驱动装不上连启动PS都有提示问题，很不爽，而且蓝牙也无法使用。intel的公版蓝牙看来有问题呀。

设备问题无法解决，就这样用吧，看来win服务器版的安全限制真的很厉害，希望能保证机器正常使用就好。

## 20号
开证明又被狗东西气到了，无耻小人连证明都要带上自己的利益。也不知道到底是哪个在耍我，我对自己的发展真是无能为力。

晚上看到了一个很有趣的软路由系统ros chr，国外有专用的设备：https://mikrotik.com/product/hap_ac3 国内也有代理，但看上去销售并不乐观，毕竟价格有点高，[狗东上也有](https://search.jd.com/Search?keyword=hAP%20ac3&enc=utf-8&wq=hAP%20ac3&pvid=15d7794d981e45e79af042e73c170557)，我收藏了最便宜的一家。

同时发现比特币完整节点的同步总是会中断，不知道是不是国内对比特币做了专门的定向封锁。

## 21号
[微软的2012官方文档](https://docs.microsoft.com/en-us/previous-versions/windows/it-pro/windows-server-2012-R2-and-2012/)
不过已经停止支持了，毕竟现在已经在发行windows2022版了。

服务器版的设置有很多没有使用过，所以有些时候必然是要看文档的，而且server版的显卡与声卡驱动也没有安装上。

头顶锅儿卖，人情大似债，这句爷爷的话我现在才体会到呢。没钱的日子真是麻烦呀。

准备读小学的材料了，现在连入学都要填写大量的家庭信息，是有这个必要的吗？

noi.cn上的培训视频居然不能下载，我要试试火狐！

chrome的安全性还是很高的，在用ghost恢复了一次系统后，居然识别到了有插件想要修改chrome的设置，估计是想加广告之类的，chrome立刻对自己进行了重置操作，所有功能要重新登录设置才可使用，太牛了，不过也是因为太厉害了，不再支持flash了，有些旧网站就很明显无法用chrome来访问了。

## 22号
昨晚又收到一个带冒犯字眼的邮件，然后今天就把[该项目的github](https://github.com/ryanhanwu/How-To-Ask-Questions-The-Smart-Way)取消关注了。有些技术人员们似乎喜欢把一些简单的问题复杂化呀。

我又重新看了一下98年【对，20多年前】的一份文档https://tldp.org/HOWTO/Unix-and-Internet-Fundamentals-HOWTO/index.html ，中文网站现在还有几个能找到20多年前的资料？黑客精神的真谛到哪儿去了呢？http://www.catb.org/~esr/faqs/hacker-howto.html 我们只能阅读英文资料才能找到吧。这位大教堂与市集的作者提供了非常丰富的[howto系列文章](http://www.catb.org/~esr/faqs/),我在这上面可没看到那种简中的傲慢。

这台dell 3046m的噪音真的太大了，我要买台mac mini把它换掉！而且看上去大部噪音来自风扇和硬盘。

动态域名功能又SB一样的使用了代理服务器的Ip，只好把梯子换成了GFW模式。
当时在小米r3p的openwrt上安装的这个功能：https://openwrt.org/docs/guide-user/services/ddns/client

现在搞的主路由还没办法离开这个二级路由了。

主路由是lede的源，版本也较低，反而是小米刷的二级路由版本较新的原版openwrt。家中的网络也是可以用混乱来表达了。
...
下面是主路由原版自带的源：
        ...
        src/gz openwrt_ddnsto https://mirrors.cloud.tencent.com/lede/snapshots/packages/x86_64/ddnsto
...
所以就按[网上的教程](https://hellodk.cn/post/246)换成了下面的源：    
...
稀里糊涂的把ddns的更新又搞好了，现在主路由可以直接更新DDNS了。
vscode的AI在2012上不能使用吗？？

总算注册成功了一个美国区的paypal，苹果ID有了自己的支付方式了，虽然实际还是走的中国visa卡支付，好歹现在有自己的支付渠道了，不知道会不会被识别出来，希望paypal不会与苹果共享支付信息吧。

然后油管博主用的https://browser.geekbench.com/ 这个硬件记录网站居然是要收费的，还不便宜，毕竟是真正的全平台支持，连linux和ios都支持了。算了，我还是继续用免费的cpu-id软件来记录设备吧，只是不支持苹果的设备而已。

## 23号
昨晚又是3点入睡，早起还带宝宝看牙，乳牙滞留出现了双排牙的问题，头疼中。

回家从早上9点睡到下午3点，我这时间真是用的乱了。

https://my.visualstudio.com/ 微软的这个控制台真牛呀。订阅下载的官方位置

## 24号
睡到10点起床，精神好多了，昨天的低烧不知道是不是因为太累了的原因。

电脑是不是应该设置一个9点半的定时关机。

在油管上看到了一个ip6的设置方法，试了一下，好像真的是可以使用的！也注册了一个ip6的：http://anqing.v6.navy/ 测试的网站是：https://dynv6.com/

不过配置的方法跟ip4有些不一样，要研究一番才行了，希望cloudfale能支持域名动态解释成ip6呀！那就太棒了！！！

linux版的bitcoin node安装看上去比windows简单多了：

                curl https://bitnodes.io/install-full-node.sh | sh

执行一下这个就行了。

突发停水就这么遇到了，今年还真是狗屁呀。多少年没遇到过的事了。一小时后，供水恢复了，然而十分钟前96557还打电话来说今天可能无法修复。内部沟通的效率堪忧呀。

## 25号
孩子一年级入学报名了，见识到了名校的人山人海呀！

由于入学面试的时候宝宝表现的不好，晚上不得不听子母的安排，请好朋友对宝宝进行一对一的辅导了。晚上喝了点白酒，又是难以入睡的到2点多才睡着。

## 26号
再到学校强调要求交流，不过还是没有明确的答复，不知道会不会有个好结果。

邀请老领导午餐一聚被回绝了，想想也是，自己的水平也是达不到跟老领导聚餐的层次吧。算了，就这样吧。

总算找到一个不错的投屏软件：https://letsview.com/zh/ 同时支持安卓与苹果，而且可以走air play的苹果都不用安装手机软件就行了。

~~意外的发现本地电信真的没有封ipv6的80和443端口耶！！而且freeddns.org直接使用的是ip6的动态映射，这就牛X了！~~

高兴早了，根本就是封掉的，只是内网解析回来了才可以直接访问而已。++

## 27号
老娘晚上天天看电视搞感冒发烧了，吵死了要去医院，买了退烧和感冒药让她先吃两天再说吧。

一个“烟花”台风吓的本地学校全部被停课，实际这台风已经北上了，对本地并没有什么大的影响。

下午宝宝又在家看一下午的动画片，受不了呀，我要买个路由器限制她这样上网了。

看中了[这个tplink的AX1800路由器](https://item.jd.com/100011729544.html)，wifi6，非智能，手机可控，差不多够用了。

然后继续对比着手机笔记软件，国外的notion功能强大，可是我已经没有什么学习的动力了，国内的有道云笔记简单的多，价格来看，差距倒是不大,都是月租制的,[notion月租费用](https://www.notion.so/pricing)4美元一个月，[有道云笔记](http://note.youdao.com/charge/setMeal.html)172人民币一年。

我带宝宝其实也没有足够的耐心与方法，更多的好像是一种放任。果然要培养一个成功的人真的很难，普通家庭基本没有足够的精力与能力吧。

有人做事就是比我好太多了吧。我的格局还是小了点。

http://www.hostbuf.com/c/131.html 这个ssh工具也很被推荐，可惜国产的我不太信。

## 28号
下载msdn镜像时使用微云发现离线功能与速度都远超百度盘，看来腾讯还是很牛的呀。

## 30号
到老代那坐一早上，听其夸夸其谈，看来也没什么好说的了。
中午他请吃一餐，喝3瓶啤酒，安睡一下午。

## 31号
vs code虽然可以集成git，但使用上的感觉还是不如github自己的客户端直接。

看到一个免费域名服务：https://nic.eu.org/ 号称96年就开始的，但我一直不知道，而且看上去提供的也就是三级域名，不知道为什么有人会认为这是顶级域名呀。

中午陪老丈人喝一瓶啤酒，下午又睡到6点才醒。